...
## انتخاب دسته از پنج نتیجهٔ اول
ابزار MCP جدید `detect_categories(query)` پنج نتیجهٔ اول را می‌گیرد و دستهٔ
هر محصول را از جزئیات دریافت می‌کند. فقط توافق هر پنج محصول باعث انتخاب
خودکار `selected_category` می‌شود. اختلاف یا دستهٔ نامشخص نیاز به انتخاب
کاربر دارد؛ گزینه‌ها در `categories` هستند. سپس `browse_category(slug, page)`
برای مرور دسته استفاده می‌شود. `product_details` هم اکنون دسته را برمی‌گرداند.

رابط وب از `/api/categories?q=...` استفاده می‌کند؛ در حالت اختلاف، دسته‌ها
زیر جستجو نمایش داده می‌شوند. انتخاب دسته متن جستجوی اولیه را از درخواست
حذف می‌کند. پارامتر `category=slug` در search/filter/rank پشتیبانی می‌شود.
مرور عادی پیش‌فرض است و **تمام صفحات قابل‌دسترسی API دسته** را می‌توان ورق زد؛
رتبه‌بندی کیفیت و فیلتر محلی همچنان روی نمونهٔ حداکثر ۱۰ صفحه‌اند، نه همهٔ
محصولات دسته. کش هر دسته مستقل است. این فرایند نیاز به LLM ندارد.

تست‌ها: `.venv/bin/python -m unittest discover -v`
